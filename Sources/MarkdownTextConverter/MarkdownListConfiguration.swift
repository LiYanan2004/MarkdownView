...
package struct MarkdownListConfiguration: Hashable, @unchecked Sendable {
    ...
    package init(
        leadingIndentation: CGFloat = 12,
        unorderedListMarker: AnyUnorderedListMarkerProtocol = AnyUnorderedListMarkerProtocol(.bullet),
        orderedListMarker: AnyOrderedListMarkerProtocol = AnyOrderedListMarkerProtocol(.increasingDigits)
    ) {
        self.leadingIndentation = leadingIndentation
        self.unorderedListMarker = unorderedListMarker
        self.orderedListMarker = orderedListMarker
    }

    package init(presentationConfiguration: MarkdownPresentation.MarkdownListConfiguration) {
        leadingIndentation = presentationConfiguration.leadingIndentation
        unorderedListMarker = AnyUnorderedListMarkerProtocol(
            PresentationUnorderedListMarker(presentationConfiguration.unorderedListMarker)
        )
        orderedListMarker = AnyOrderedListMarkerProtocol(
            PresentationOrderedListMarker(presentationConfiguration.orderedListMarker)
        )
    }
}

private struct PresentationOrderedListMarker: MarkdownOrderedListMarkerProtocol {
    private var marker: AnyHashable

    var monospaced: Bool {
        (marker.base as! any MarkdownPresentation.MarkdownOrderedListMarkerProtocol).monospaced
    }

    init(_ marker: some MarkdownPresentation.MarkdownOrderedListMarkerProtocol) {
        self.marker = AnyHashable(marker)
    }

    func marker(at index: Int, listDepth: Int) -> String {
        (marker.base as! any MarkdownPresentation.MarkdownOrderedListMarkerProtocol)
            .marker(at: index, listDepth: listDepth)
    }
}

private struct PresentationUnorderedListMarker: MarkdownUnorderedListMarkerProtocol {
    private var marker: AnyHashable

    var monospaced: Bool {
        (marker.base as! any MarkdownPresentation.MarkdownUnorderedListMarkerProtocol).monospaced
    }

    init(_ marker: some MarkdownPresentation.MarkdownUnorderedListMarkerProtocol) {
        self.marker = AnyHashable(marker)
    }

    func marker(listDepth: Int) -> String {
        (marker.base as! any MarkdownPresentation.MarkdownUnorderedListMarkerProtocol)
            .marker(listDepth: listDepth)
    }
}
