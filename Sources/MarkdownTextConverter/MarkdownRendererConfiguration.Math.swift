...
extension MarkdownRendererConfiguration {
    package struct Math: Sendable, Hashable {
        ...
        package init(context: MDMathContext? = nil) {
        ...
        package init(presentationConfiguration: MarkdownPresentation.MarkdownRendererConfiguration.Math) {
            self.context = presentationConfiguration.context
        }
